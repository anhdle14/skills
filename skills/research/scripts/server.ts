...
const OPCODES = { TEXT: 0x01, CLOSE: 0x08, PING: 0x09, PONG: 0x0A } as const;
...
const MIME_TYPES: Record<string, string> = {
  ...
  ".ts": "application/typescript", ".json": "application/json", ".png": "image/png",
...
const helperScript = fs.readFileSync(path.join(__dirname, "helper.ts"), "utf-8");
...
function getNewestScreen(): string | null {
...
function handleRequest(req: IncomingMessage, res: ServerResponse): void {
  ...
  if (req.method === "GET" && url === "/") {
    ...
    res.end(html);
  } else if (req.method === "GET" && url.startsWith("/files/")) {
...
function startServer(): void {
  ...
  if (ownerPid) {
    try {
      ...
      if (!(typeof e === "object" && e !== null && "code" in e && e.code === "EPERM")) {
...
export { computeAcceptKey, decodeFrame, encodeFrame, OPCODES, startServer };
