...
Covers non-technical prose only. Developer-facing documents belong to `/tech-writing`.
...
A report, essay, or analysis piece runs the same shaping loop with three changes:

1. Lead with the finding, not the journey. The opening states what you concluded; the body earns it.
2. Every number, quote, date, and attribution traces back to the raw material. If the pile does not contain it, it does not enter the draft - write `TODO(unverified): <question>` and move on.
3. Close with implication: what the reader should now believe, decide, or do.

Structure to reach for when nothing better presents itself: finding, evidence, complication or counter-case, implication.
