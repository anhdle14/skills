...
- [ ] **Linter / formatter configs** — .eslintrc, .prettierrc, ruff.toml,
      rustfmt.toml, .editorconfig. These rules belong in the tool, not CLAUDE.md.
...
- `./CLAUDE.md` — project root, committed; the default target.
- `./.claude/CLAUDE.md` — same scope, keeps the root tidy; valid alternative.
- Nested `path/to/pkg/CLAUDE.md` — monorepo per-package; loads on demand and
  **inherits** ancestors, so state only what differs from the parent.
- `~/.claude/CLAUDE.md` — user-level, applies across all projects.
- `CLAUDE.local.md` — personal, uncommitted overrides; add to `.gitignore`.
...
- Durable, discovered mistakes can be appended later via the `#` / `/memory`
  shortcut; auto-memory (`MEMORY.md`) captures learnings, so CLAUDE.md need not.
