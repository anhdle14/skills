---
name: claude-md
description: Create or replace the per-folder agent context file that gets read on every turn, holding it to the 80-line budget where every line prevents a mistake. Use when user wants to create, write, generate, bootstrap, or rewrite a CLAUDE.md / AGENTS.md / agent context file, or onboard an agent to a repo.
...
filename: Claude reads `CLAUDE.md`, not `AGENTS.md` — if only `AGENTS.md`
exists, create a `CLAUDE.md` that does `@AGENTS.md` (or symlink). Check for
...
Linter/formatter rules belong in the tool config, **never** in CLAUDE.md.
...
context (reserve `@` only for AGENTS.md interop).
